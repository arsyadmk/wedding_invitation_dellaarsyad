...
export const guest = (() => {
    ...
    const showGuestName = () => {
        ...
        if (name) {
            ...
            const template = `<small class="font-lora mt-0 mb-1 mx-0 p-0">${util.escapeHtml(guestName?.getAttribute('data-message'))}</small><p class="font-lora m-0 p-0" style="font-size: 1.25rem">${util.escapeHtml(name)}</p>`;
